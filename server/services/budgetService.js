...
class BudgetService {
        ...
        where: {
          OR: [
            where,
            {
              assignments: {
                some: {
                  userId: options.userId // Filter by assigned user
                }
              }
            }
          ]
  ...
  formatBudget(budget) {
    return {
      ...budget,
      amount: new Decimal(budget.amount).toFixed(2),
   ...
   */
  async assignBudget(budgetId, userId, assignedBy) {
    // Check if budget exists
    const budget = await this.prisma.budget.findUnique({
      where: { id: budgetId }
    });
    ...
    // Create assignment
    const assignment = await this.prisma.budgetAssignment.upsert({
      where: {
        budgetId_userId: {
          budgetId,
          userId
      ...
      update: {
        assignedBy // Update who assigned it if already assigned
      }
    });
   ...
   */
  async unassignBudget(budgetId, userId) {
    await this.prisma.budgetAssignment.delete({
      where: {
        budgetId_userId: {
          budgetId,
          userId
    ...
    return { success: true, message: 'Budget unassigned successfully' };
  }
